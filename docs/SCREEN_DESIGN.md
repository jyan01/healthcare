...
| 상태 | HTML 샘플 생성 완료 — `demo/patient-detail-web.html`(React 웹, 반응형), `demo/patient-detail-mobile.html`(React Native, 세로형). 8개 지표 모두 실시간 스트리밍 라인차트(크로스헤어 툴팁 포함)로 구현, 심박/혈압/혈당은 상태뱃지(정상·주의·높음) 표시. 보유 질병은 프로필 카드에 칩으로, 진단 메모(`diagContent`)가 있는 질병은 별도 카드에 목록으로 표시. **[고도화]** 상단에 기간(startAt~endAt) 선택 조회 기능, "AI 소견 요약" 버튼(health-ai RAG Agent에 최근 데이터+보유질환을 컨텍스트로 전달해 요약 생성) 추가 |
